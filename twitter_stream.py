...
with open(fName, 'w') as f:
    while tweetCount < maxTweets:
        try:
            if (max_id <= 0):
                if (not sinceId):
                    new_tweets = api.search(q='$NFLX', lang='en', count=tweetsPerQry, since='2017-4-17', until='2017-4-18')
                else:
                    new_tweets = api.search(q='$NFLX', lang='en', count=tweetsPerQry,
                                            since_id=sinceId, since='2017-4-17', until='2017-4-18')
            else:
                if (not sinceId):
                    new_tweets = api.search(q='$NFLX', lang='en', count=tweetsPerQry,
                                            max_id=str(max_id - 1), since='2017-4-17', until='2017-4-18')
                else:
                    new_tweets = api.search(q='$NFLX', lang='en', count=tweetsPerQry,
                                            max_id=str(max_id - 1),
                                            since_id=sinceId, since='2017-4-17', until='2017-4-18')
